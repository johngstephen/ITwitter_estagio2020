...
def connect(username, created_at, tweet, retweet_count, place, location):
    ...
    try:
        con = mysql.connector.connect(host='localhost',
                                      database='twitter', user='root', password='', charset='utf8')
        ...
        if con.is_connected():
            ...
            querySave = "INSERT INTO twitter (username, created_at, tweet, retweet_count,place, location) VALUES (%s, %s, %s, %s, %s, %s)"
            cursor.execute(querySave, (username, created_at, tweet, retweet_count, place, location))
            con.commit()
            # queryDelete ="DELETE * FROM twitter WHERE ID = 6"
            # cursor.execute(queryDelete, (username, created_at, tweet, retweet_count, place, location))
            # con.commit()
